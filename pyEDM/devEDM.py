...
def Prediction( embedding, colNames, target, args ):
    '''
    Primary interface and wrapper for SMapProjection() [Sugihara, 1994].
    ...
    if args.k_NN <= 0 :  # JP! devEDM sets default knn = -1   pyEDM uses 0 !!
    ...
    if 'smap' in args.method.lower():
        ...
        Predictions, ConstPredict, Coeff = \
    ...
    return ( rho, rmse, mae, header, output, smap_output, coef_header )
...
def SMapProjection( libraryMatrix, predictMatrix, target, const_target,
    ...
    Tikhonov   = False
    ElasticNet = False
    if "svd" in args.solver.lower() :
        SVD = True
    elif "tikhonov" in args.solver.lower() :
        Tikhonov = True
    elif "elasticnet" in args.solver.lower() :
        ElasticNet = True
    else:
        raise RuntimeError( "SMapProjection() Invalid solver." )
    ...
    return ( predictions, const_predict, coefficients )
...
def ElasticNet_( A, b, args ) :
    ...
    en = ElasticNetCV( l1_ratio      = args.ElasticNetAlpha,  # [ 0.01 ... 1 ]
                       eps           = 0.001,
                       n_alphas      = 100,
                       alphas        = None,
                       fit_intercept = True,
    ...
    if args.verbose :
        print('ElasticNetCV l1_ratio', en.l1_ratio_, '--------------------' )
        print( 'coef',  en.coef_ )  # Parameter vector (w in the cost function)
        print( 'alpha', en.alpha_ ) # Penalization chosen by cross validation
...
def F_alpha( alpha, A, b, w, E ) :
    ...
    u, s, vT = svd( A, full_matrices = False ) # numpy.linalg.svd()
