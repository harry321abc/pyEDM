...
def ParseCmdLine():
    ...
    parser.add_argument('-en', '--ElasticNetAlpha', nargs = '+', # L1_ratio's
                        dest   = 'ElasticNetAlpha', type = float,
                        action = 'store', default = None,
                        help = 'Elastic Net L1_ratios in S-Map.')
...
def AdjustArgs( args ):
    ...
    if "smap" in args.method.lower() :
        ...
        if "elasticnet" in args.solver.lower() :
            try:
            ...
            if isinstance( args.ElasticNetAlpha, list ) or \
               isinstance( args.ElasticNetAlpha, ndarray ):
                if min( args.ElasticNetAlpha ) <= 0.01 or \
                   max( args.ElasticNetAlpha ) >  1 :
                    raise RuntimeError("ParseCmdLine() ElasticNetCV L1_ratio"+\
                                       " out of bounds [0.01 : 1]." +\
                                       " See scikit-learn.org")
            elif args.ElasticNetAlpha :
                if args.ElasticNetAlpha <= 0.01 or \
